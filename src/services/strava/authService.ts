...
import { StravaConnectionResult, StravaAuthUrlResult, StravaOperationResult } from "./types";

/**
 * Checks if a user is connected to Strava
 */
...
/**
 * Checks a user's Strava connection status
 */
...
/**
 * Connects a user to Strava by generating an authorization URL
 */
...
/**
 * Disconnects a user from Strava by removing their tokens
 */
export const disconnectFromStrava = async (userId: string): Promise<StravaOperationResult> => {
  try {
...
/**
 * Updates the Strava edge function (placeholder for future use)
 */
export const updateStravaEdgeFunction = async (): Promise<StravaOperationResult> => {
  return { success: true, error: null };
};
